...
ambiguous_transfer::Node ambiguousGraph(
...
void ambiguous_transfer::getOuts(vector<string>* finalOuts,
...
void ambiguous_transfer::getOuts(vector<string>* finalOuts,
	...
	for (unsigned i = 0; i < ambigInfo.size(); i++) {
		ambigMap.insert(
				pair<unsigned, ambiguous_transfer::AmbigInfo>(
			...
			if (nodes.size() > 1 && ambigMap.count(j)) {
				...
				if (ambigInfo[i].firTokId == j) {
					...
					putCombination(&combNodes,
							vector<ambiguous_transfer::Node>(
...
void putCombination(vector<vector<ambiguous_transfer::Node> >* combinations,
...
vector<vector<ambiguous_transfer::Node> > putCombinations(
		vector<vector<ambiguous_transfer::Node> > combinations,
...
void getCombinations(ambiguous_transfer::Node root,
		vector<ambiguous_transfer::Node> path,
...
void ambiguous_transfer::normaliseWeights(vector<float>* weights,
		vector<ambiguous_transfer::AmbigInfo> ambigInfo) {
...
void ambiguous_transfer::getAmbigInfo(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules,
		map<unsigned, vector<ambiguous_transfer::Node> > nodesPool,
	...
	*combNum = 0;
	for (unsigned tokId = 0; tokId < tokenRules.size();) {
		...
		if (nodesPool[tokId].size() > 1) {
			ambiguous_transfer::AmbigInfo ambig = ambiguous_transfer::AmbigInfo(
					tokId, maxPat);

			ambiguous_transfer::Node dummy = ambiguousGraph(tokenRules,
					nodesPool, tokId, maxPat);
...
map<unsigned, vector<ambiguous_transfer::Node> > ambiguous_transfer::getNodesPool(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules) {
	map<unsigned, vector<ambiguous_transfer::Node> > nodesPool;
	for (map<unsigned, vector<pair<unsigned, unsigned> > >::iterator it =
		...
		for (unsigned i = 0; i < rules.size(); i++) {
			...
			ambiguous_transfer::Node node = ambiguous_transfer::Node(tokenId,
					ruleId, patNum);
...
bool sortParameter(pair<unsigned, unsigned> a, pair<unsigned, unsigned> b) {
		...
		for (unsigned i = 0; i < rulesApplied[rule].size(); i++) {
			...
			if (xml_operations::xml_operations::getAttValUnsg(rule, ID) == 0)
				...
				output = ruleExe(rule, &slAnalysisTokens, &tlAnalysisTokens,
						attrs, lists, vars, spaces, firstMatTok, localeId); // first pattern index
...
void ambiguous_transfer::ruleOuts(
		map<unsigned, map<unsigned, string> >* ruleOuts,
		map<unsigned, vector<pair<unsigned, unsigned> > >* tokenRules,
		vector<string> slTokens, vector<vector<string> > slTags,
		vector<string> tlTokens, vector<vector<string> > tlTags,
	...
	for (xmlNode* child = xml_operations::getFirstChild(action); child; child =
		...
		if (childName == LET) {
			...
			result = choose(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, localeId, paramToPattern);
...
vector<string> out(xmlNode* out, vector<vector<string> >* slAnalysisTokens,
	...
	for (xmlNode* child = xml_operations::getFirstChild(out); child; child =
		...
		if (childName == B) {
			result.push_back(
					b(child, spaces, firPat, localeId, paramToPattern));
	...
	for (xmlNode* child = xml_operations::getFirstNext(tags); child; child =
		...
		if (childName == LU) {
			...
			vector<string> result = concat(child, slAnalysisTokens,
					tlAnalysisTokens, attrs, vars, spaces, firPat, localeId,
...
vector<string> callMacro(xmlNode* callMacroNode,
	...
	for (xmlNode* child = xml_operations::getFirstChild(macro); child; child =
		...
		if (childName == CHOOSE)
			result = choose(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, localeId, newParamToPattern);
...
bool equal(xmlNode* equal, vector<vector<string> >* slAnalysisTokens,
	...
	if (firstName == CLIP) {
		firstResult = clip(firstChild, slAnalysisTokens, tlAnalysisTokens,
				attrs, vars, spaces, firPat, localeId, paramToPattern);
		...
		firstResult.push_back(
				b(firstChild, spaces, firPat, localeId, paramToPattern));
	...
	if (secondName == CLIP) {
		...
		secondResult.push_back(
				b(secondChild, spaces, firPat, localeId, paramToPattern));
	...
	for (xmlNode* child = xml_operations::getFirstChild(chooseNode); child;
		...
		if (childName == WHEN) {
			...
			condition = test(testNode, slAnalysisTokens, tlAnalysisTokens,
					attrs, lists, vars, spaces, firPat, localeId,
					paramToPattern);
		} else {
			// otherwise
			condition = true;
...
bool in(xmlNode* inNode, vector<vector<string> >* slAnalysisTokens,
	...
	xmlNode* listNode = xml_operations::getFirstChild(firstChild);
...
void let(xmlNode* let, vector<vector<string> >* slAnalysisTokens,
	...
	if (firstName == VAR) {
		...
		cout << "varname=" << varName << " , value=" << (*vars)[varName]
				<< endl;
	...
	for (xmlNode* node = xml_operations::getFirstChild(concat); node; node =
		...
		if (nodeName == CLIP) {
			result = clip(node, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
					spaces, firPat, localeId, paramToPattern, tags);
	...
	for (xmlNode* child = xml_operations::getFirstChild(append); child; child =
		...
		if (childName == CLIP) {
			...
			string caseOfResult = caseOf(child, slAnalysisTokens,
					tlAnalysisTokens, localeId, paramToPattern);
...
string b(xmlNode* b, vector<string> spaces, unsigned firPat, string localeId,
		map<unsigned, unsigned> paramToPattern) {
...
void modifyCase(xmlNode* modifyCase, vector<vector<string> >* slAnalysisTokens,
	...
	if (childName == VAR) {
		...
		if (Case == aa)
			...
			(*vars)[varName] = CLExec::FirLetUpperCase((*vars)[varName],
					localeId);
		...
		if (part == LEM) {
			if (Case == aa)
				(*analysisTokens)[pos][0] = CLExec::toLowerCase(
						(*analysisTokens)[pos][0], localeId);
			else if (Case == AA)
				(*analysisTokens)[pos][0] = CLExec::toUpperCase(
						(*analysisTokens)[pos][0], localeId);
			else if (Case == Aa)
