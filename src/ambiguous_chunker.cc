...
void AmbiguousChunker::lexFormsTokenizer(vector<string>* slTokens,
...
void AmbiguousChunker::matchCats(map<unsigned, vector<string> >* catsApplied,
		vector<string> slTokens, vector<vector<string> > tags,
		xmlNode* transfer) {
	xmlNode* section_def_cats = xml_operations::getChild(transfer,
			SECTION_DEF_CATS);

	for (xmlNode* def_cat = xml_operations::getFirstChild(section_def_cats);
			def_cat; def_cat = xml_operations::getFirstNext(def_cat)) {

		for (xmlNode* cat_item = xml_operations::getFirstChild(def_cat);
				cat_item; cat_item = xml_operations::getFirstNext(cat_item)) {
			...
			for (unsigned x = 0; x < slTokens.size(); x++) {
				...
				if (!xml_operations::getAttVal(cat_item, LEMMA).empty()) {
					if (xml_operations::getAttVal(cat_item, LEMMA)
							!= slTokens[x]) {
						continue;
					}
				}
				...
				if (i == tokTags.size()
					...
					if (catsApplied->find(x) != catsApplied->end())
						catsApplied->at(x).push_back(s);
					else
						catsApplied->insert(
								pair<unsigned, vector<string>>(x,
										vector<string>()));
				}
			}
		}
...
void AmbiguousChunker::matchRules(
	...
	for (xmlNode* rule = xml_operations::getFirstChild(section_rules); rule;
			rule = xml_operations::getFirstNext(rule)) {
		...
		for (xmlNode* pattern_item = xml_operations::getFirstChild(pattern);
				pattern_item;
		...
		for (unsigned i = 0;
			...
			for (unsigned j = 0; j < pattern_items.size(); j++) {
				...
				for (unsigned k = 0; k < cats.size(); k++) {
					...
					if (xml_operations::getAttVal(pattern_items[j], N)
							== cats[k]) {
...
bool sortParameter1(vector<string> a, vector<string> b) {
	return (a.size() > b.size());
}

map<string, vector<vector<string> > > AmbiguousChunker::getAttrs(
		xmlNode* transfer) {
	map<string, vector<vector<string> > > attrs;
	xmlNode* section_def_attrs = xml_operations::getChild(transfer,
			SECTION_DEF_ATTRS);

	for (xmlNode* def_attr = xml_operations::getFirstChild(section_def_attrs);
			def_attr; def_attr = xml_operations::getFirstNext(def_attr)) {

		vector<vector<string> > allTags;
		for (xmlNode* attr_item = xml_operations::getFirstChild(def_attr);
				attr_item;
...
map<string, string> AmbiguousChunker::getVars(xmlNode* transfer) {
	map<string, string> vars;

	xmlNode* section_def_vars = xml_operations::getChild(transfer,
			SECTION_DEF_VARS);
	if (section_def_vars)
		for (xmlNode* def_var = xml_operations::getFirstChild(section_def_vars);
				def_var; def_var = xml_operations::getFirstNext(def_var)) {

			vars[xml_operations::getAttVal(def_var, N)] =
					xml_operations::getAttVal(def_var, V);
		}

	return vars;
}

map<string, vector<string> > AmbiguousChunker::getLists(xmlNode* transfer) {
	map<string, vector<string> > lists;

	xmlNode* section_def_lists = xml_operations::getChild(transfer,
			SECTION_DEF_LISTS);
	if (section_def_lists)
		for (xmlNode* def_list = xml_operations::getFirstChild(
				section_def_lists); def_list; def_list =
				xml_operations::getFirstNext(def_list)) {

			vector<string> list;
			for (xmlNode* list_item = xml_operations::getFirstChild(def_list);
					list_item;
...
void AmbiguousChunker::getOuts(vector<string>* finalOuts,
...
void AmbiguousChunker::getOuts(vector<string>* finalOuts,
	...
	for (unsigned i = 0; i < ambigInfo.size(); i++) {
		ambigMap.insert(
				pair<unsigned, AmbiguousChunker::AmbigInfo>(
						ambigInfo[i].firTokId, ambigInfo[i]));
	}
			...
			if (nodes.size() > 1 && ambigMap.count(j)) {
				...
				if (ambigInfo[i].firTokId == j) {
					...
					putCombination(&combNodes,
							vector<AmbiguousChunker::Node>(
									combinations[0].begin() + 1,
									combinations[0].end()));
...
AmbiguousChunker::Node ambiguousGraph(
...
void putCombination(vector<vector<AmbiguousChunker::Node> >* combinations,
...
vector<vector<AmbiguousChunker::Node> > putCombinations(
		vector<vector<AmbiguousChunker::Node> > combinations,
	...
	for (unsigned i = 0; i < combinations.size(); i++) {
		for (unsigned j = 0; j < nestedcombinations.size(); j++) {
			vector<AmbiguousChunker::Node> newcombination = vector<
					AmbiguousChunker::Node>(combinations[i]);
...
void putOut(vector<string>* outputs, string output, unsigned tokenIndex,
		vector<string> spaces) {
...
vector<string> putOuts(vector<string> outputs, vector<string> nestedOutputs) {
...
void getCombinations(AmbiguousChunker::Node root,
		vector<AmbiguousChunker::Node> path,
...
void AmbiguousChunker::normaliseWeights(vector<float>* weights,
		vector<AmbiguousChunker::AmbigInfo> ambigInfo) {
...
void getMaxPat(int curMaxPat, unsigned curToken,
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules,
		unsigned* count) {
	if (curMaxPat == 0)
		return;
...
void AmbiguousChunker::getAmbigInfo(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules,
		map<unsigned, vector<AmbiguousChunker::Node> > nodesPool,
	...
	*combNum = 0;
	for (unsigned tokId = 0; tokId < tokenRules.size();) {
		...
		if (nodesPool[tokId].size() > 1) {
			AmbiguousChunker::AmbigInfo ambig = AmbiguousChunker::AmbigInfo(
					tokId, maxPat);

			AmbiguousChunker::Node dummy = ambiguousGraph(tokenRules, nodesPool,
					tokId, maxPat);
			getCombinations(dummy, vector<AmbiguousChunker::Node>(),
					&ambig.combinations);
...
map<unsigned, vector<AmbiguousChunker::Node> > AmbiguousChunker::getNodesPool(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules) {
	map<unsigned, vector<AmbiguousChunker::Node> > nodesPool;
	for (map<unsigned, vector<pair<unsigned, unsigned> > >::iterator it =
		...
		for (unsigned i = 0; i < rules.size(); i++) {
			...
			AmbiguousChunker::Node node = AmbiguousChunker::Node(tokenId,
					ruleId, patNum);
...
string noRuleOut(vector<string> analysis) {
	vector<string> out;
	// unknown word
	if (analysis[0][0] == '*')
...
void nestedRules(vector<string> tlTokens, string output,
		vector<pair<unsigned, unsigned> > curNestedRules,
	...
	for (unsigned i = 0; i < rulesApplied.size(); i++) {
		...
		if (patNum <= curPatNum) {
			...
			newCurNestedRules.push_back(
					pair<unsigned, unsigned>(rule, curTokIndex));
			...
			if (curPatNum == patNum) {
				...
				nestedRules(tlTokens, output, newCurNestedRules, outputs,
...
bool sortParameter2(pair<unsigned, unsigned> a, pair<unsigned, unsigned> b) {
	return (a.second > b.second);
}

void pushDistinct(map<unsigned, vector<pair<unsigned, unsigned> > >* tokenRules,
	...
	sort((*tokenRules)[tlTokInd].begin(), (*tokenRules)[tlTokInd].end(),
			sortParameter2);
}

// for debugging
void printNodeAttrs(xmlNode* node) {
//	cout << node->name << endl;
...
void ruleOuts(map<unsigned, map<unsigned, string> >* ruleOuts,
		map<unsigned, vector<pair<unsigned, unsigned> > >* tokenRules,
	...
	for (map<xmlNode*, vector<pair<unsigned, unsigned> > >::iterator it =
		...
		for (unsigned i = 0; i < rulesApplied[rule].size(); i++) {
			...
			for (unsigned tokInd = firstMatTok; tokInd < firstMatTok + patNum;
					tokInd++) {
				vector<string> slAnalysisToken = formatTokenTags(
						slTokens[tokInd], slTags[tokInd]);

				slAnalysisTokens.push_back(slAnalysisToken);

				vector<string> tlAnalysisToken = formatTokenTags(
			...
			if (xml_operations::xml_operations::getAttValUnsg(rule, ID) == 0)
				...
				output = ruleExe(rule, &slAnalysisTokens, &tlAnalysisTokens,
						attrs, lists, vars, spaces, firstMatTok); // first pattern index
...
void AmbiguousChunker::ruleOuts(map<unsigned, map<unsigned, string> >* ruleOuts,
		map<unsigned, vector<pair<unsigned, unsigned> > >* tokenRules,
		vector<string> slTokens, vector<vector<string> > slTags,
		vector<string> tlTokens, vector<vector<string> > tlTags,
		map<xmlNode*, vector<pair<unsigned, unsigned> > > rulesApplied,
	...
	for (map<xmlNode*, vector<pair<unsigned, unsigned> > >::iterator it =
		...
		for (unsigned i = 0; i < rulesApplied[rule].size(); i++) {
			...
			(*ruleOuts)[xml_operations::getAttValUnsg(rule, ID)][firstMatTok] =
					str;
		}
	}
...
vector<string> ruleExe(xmlNode* rule, vector<vector<string> >* slAnalysisTokens,
	...
	for (xmlNode* child = xml_operations::getFirstChild(action); child; child =
		...
		if (childName == LET) {
			let(child, slAnalysisTokens, tlAnalysisTokens, attrs, vars, spaces,
					firPat, paramToPattern);
		} else if (childName == CHOOSE) {
			result = choose(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		} else if (childName == CALL_MACRO) {
			result = callMacro(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		} else if (childName == OUT) {
			result = out(child, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
					spaces, firPat, paramToPattern);
		}

		else if (childName == MODIFY_CASE)
			modifyCase(child, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
					spaces, firPat, paramToPattern);

		else if (childName == APPEND)
			append(child, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
					spaces, firPat, paramToPattern);
...
vector<string> out(xmlNode* out, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
	...
	for (xmlNode* child = xml_operations::getFirstChild(out); child; child =
		...
		if (childName == B) {
			...
			result = chunk(child, slAnalysisTokens, tlAnalysisTokens, attrs,
...
vector<string> chunk(xmlNode* chunkNode,
	...
	for (xmlNode* tag = xml_operations::getFirstChild(tags); tag; tag =
		...
		if (childName == CLIP) {
			result = clip(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					vars, spaces, firPat, paramToPattern);
		} else if (childName == CONCAT) {
			result = concat(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					vars, spaces, firPat, paramToPattern);
			...
			result.push_back(
					caseOf(child, slAnalysisTokens, tlAnalysisTokens,
							paramToPattern));
		} else if (childName == GET_CASE_FROM) {
			result.push_back(
					getCaseFrom(child, slAnalysisTokens, tlAnalysisTokens,
	...
	for (xmlNode* child = xml_operations::getFirstNext(tags); child; child =
		...
		if (childName == LU) {
			...
			vector<string> result = concat(child, slAnalysisTokens,
					tlAnalysisTokens, attrs, vars, spaces, firPat,
			...
			for (xmlNode* lu = xml_operations::getFirstChild(child); lu; lu =
				...
				vector<string> result = concat(lu, slAnalysisTokens,
						tlAnalysisTokens, attrs, vars, spaces, firPat,
						paramToPattern, tagsResult);
				output.insert(output.end(), result.begin(), result.end());
...
vector<string> callMacro(xmlNode* callMacroNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
	...
	unsigned i = 1;
	for (xmlNode* with_param = xml_operations::getFirstChild(callMacroNode);
			with_param; with_param = xml_operations::getFirstNext(with_param)) {
		unsigned pos = xml_operations::xml_operations::getAttValUnsg(with_param,
				POS);
	...
	for (xmlNode* child = xml_operations::getFirstChild(macro); child; child =
		...
		if (childName == CHOOSE)
			result = choose(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, newParamToPattern);
...
vector<string> findAttrPart(vector<string> tokenTags,
...
bool equal(xmlNode* equal, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
		map<unsigned, unsigned> paramToPattern) {
	...
	if (firstName == CLIP) {
		firstResult = clip(firstChild, slAnalysisTokens, tlAnalysisTokens,
				attrs, vars, spaces, firPat, paramToPattern);
	} else if (firstName == CONCAT) {
		firstResult = concat(firstChild, slAnalysisTokens, tlAnalysisTokens,
				attrs, vars, spaces, firPat, paramToPattern);
		...
		firstResult.push_back(
				caseOf(firstChild, slAnalysisTokens, tlAnalysisTokens,
						paramToPattern));
	} else if (firstName == GET_CASE_FROM) {
		firstResult.push_back(
				getCaseFrom(firstChild, slAnalysisTokens, tlAnalysisTokens,
						attrs, vars, spaces, firPat, paramToPattern));
	...
	if (secondName == CLIP) {
		secondResult = clip(secondChild, slAnalysisTokens, tlAnalysisTokens,
				attrs, vars, spaces, firPat, paramToPattern);
	} else if (secondName == CONCAT) {
		secondResult = concat(secondChild, slAnalysisTokens, tlAnalysisTokens,
				attrs, vars, spaces, firPat, paramToPattern);
		...
		secondResult.push_back(
				caseOf(secondChild, slAnalysisTokens, tlAnalysisTokens,
						paramToPattern));
	} else if (secondName == GET_CASE_FROM) {
		secondResult.push_back(
				getCaseFrom(secondChild, slAnalysisTokens, tlAnalysisTokens,
						attrs, vars, spaces, firPat, paramToPattern));
...
vector<string> choose(xmlNode* chooseNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
	...
	for (xmlNode* child = xml_operations::getFirstChild(chooseNode); child;
			child = xml_operations::getFirstNext(child)) {
		...
		if (childName == WHEN) {
			...
			condition = test(testNode, slAnalysisTokens, tlAnalysisTokens,
					attrs, lists, vars, spaces, firPat, paramToPattern);
		} else {
			// otherwise
			condition = true;
		}
		...
		if (condition) {
			for (xmlNode* inst = xml_operations::getFirstChild(child); inst;
					inst = xml_operations::getFirstNext(inst)) {
				vector<string> result;

				string instName = xml_operations::getName(inst);
				if (instName == CHOOSE)
					result = choose(inst, slAnalysisTokens, tlAnalysisTokens,
							attrs, lists, vars, spaces, firPat, paramToPattern);

				else if (instName == OUT)
					result = out(inst, slAnalysisTokens, tlAnalysisTokens,
							attrs, vars, spaces, firPat, paramToPattern);

				else if (instName == CALL_MACRO)
					result = callMacro(inst, slAnalysisTokens, tlAnalysisTokens,
							attrs, lists, vars, spaces, firPat, paramToPattern);

				else if (instName == LET)
					let(inst, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
							spaces, firPat, paramToPattern);

				else if (instName == MODIFY_CASE)
					modifyCase(inst, slAnalysisTokens, tlAnalysisTokens, attrs,
							vars, spaces, firPat, paramToPattern);

				else if (instName == APPEND)
					append(inst, slAnalysisTokens, tlAnalysisTokens, attrs,
							vars, spaces, firPat, paramToPattern);
...
bool test(xmlNode* test, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
	...
	if (childName == EQUAL) {
		condition = equal(child, slAnalysisTokens, tlAnalysisTokens, attrs,
				vars, spaces, firPat, paramToPattern);
	} else if (childName == AND) {
		condition = And(child, slAnalysisTokens, tlAnalysisTokens, attrs, lists,
				vars, spaces, firPat, paramToPattern);
	} else if (childName == OR) {
		condition = Or(child, slAnalysisTokens, tlAnalysisTokens, attrs, lists,
				vars, spaces, firPat, paramToPattern);
	} else if (childName == NOT) {
		condition = Not(child, slAnalysisTokens, tlAnalysisTokens, attrs, lists,
				vars, spaces, firPat, paramToPattern);
	} else if (childName == IN) {
		condition = in(child, slAnalysisTokens, tlAnalysisTokens, attrs, lists,
				vars, spaces, firPat, paramToPattern);
	}
...
bool And(xmlNode* andNode, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
	...
	for (xmlNode* child = xml_operations::getFirstChild(andNode); child; child =
		...
		if (childName == EQUAL) {
			...
			condition = And(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		} else if (childName == OR) {
			condition = Or(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		} else if (childName == NOT) {
			condition = Not(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		} else if (childName == IN) {
			condition = in(child, slAnalysisTokens, tlAnalysisTokens, attrs,
					lists, vars, spaces, firPat, paramToPattern);
		}
...
bool Or(xmlNode* orNode, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
...
bool in(xmlNode* inNode, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
...
bool Not(xmlNode* NotNode, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
...
vector<string> litTag(xmlNode* litTag) {
...
string var(xmlNode* var, map<string, string>* vars) {
	printNodeAttrs(var);
...
void let(xmlNode* let, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
		map<unsigned, unsigned> paramToPattern) {
	...
	if (firstName == VAR) {
		...
		vector<string> firstResult = clip(firstChild, slAnalysisTokens,
				tlAnalysisTokens, attrs, vars, spaces, firPat, paramToPattern);
		if (firstResult.empty())
			return;

		unsigned pos = xml_operations::xml_operations::getAttValUnsg(firstChild,
				POS);
		if (paramToPattern.size())
			pos = paramToPattern[pos];
		pos--;
		...
		for (unsigned i = 0; i < analysisToken->size(); i++) {
			if ((*analysisToken)[i] == firstResult[0]) {
				...
				analysisToken->insert(analysisToken->begin() + i,
...
vector<string> formatTokenTags(string token, vector<string> tags) {
...
vector<string> clip(xmlNode* clip, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
	...
	if (part == WHOLE) {
		...
		result.insert(result.end(), analysisToken.begin() + 1,
				analysisToken.end());
	}
	// part == "attr"
	else {
...
vector<string> concat(xmlNode* concat,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
	...
	for (xmlNode* node = xml_operations::getFirstChild(concat); node; node =
		...
		if (nodeName == CLIP) {
			result = clip(node, slAnalysisTokens, tlAnalysisTokens, attrs, vars,
					spaces, firPat, paramToPattern, tags);
			...
			result.push_back(
					getCaseFrom(node, slAnalysisTokens, tlAnalysisTokens, attrs,
							vars, spaces, firPat, paramToPattern));
		} else if (nodeName == CASE_OF) {
			result.push_back(
					caseOf(node, slAnalysisTokens, tlAnalysisTokens,
							paramToPattern));
		} else if (nodeName == B) {
...
void append(xmlNode* append, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
		map<unsigned, unsigned> paramToPattern) {
	...
	for (xmlNode* child = xml_operations::getFirstChild(append); child; child =
		...
		string childName = xml_operations::getName(child);
		if (childName == CLIP) {
			vector<string> clipResult = clip(child, slAnalysisTokens,
					tlAnalysisTokens, attrs, vars, spaces, firPat,
					paramToPattern);
			...
			vector<string> concatResult = concat(child, slAnalysisTokens,
					tlAnalysisTokens, attrs, vars, spaces, firPat,
					paramToPattern);
			result.insert(result.end(), concatResult.begin(),
					concatResult.end());
		} else if (childName == B) {
			...
			string caseOfResult = caseOf(child, slAnalysisTokens,
...
string b(xmlNode* b, vector<string> spaces, unsigned firPat,
		map<unsigned, unsigned> paramToPattern) {
...
string caseOf(xmlNode* caseOf, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<unsigned, unsigned> paramToPattern) {
...
string getCaseFrom(xmlNode* getCaseFrom,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
	...
	unsigned pos = xml_operations::xml_operations::getAttValUnsg(getCaseFrom,
			POS);
	if (paramToPattern.size())
		pos = paramToPattern[pos];
	pos--;
...
void modifyCase(xmlNode* modifyCase, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat,
		map<unsigned, unsigned> paramToPattern) {
