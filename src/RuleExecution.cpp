...
void putCombination(vector<vector<RuleExecution::Node*> >* combinations,
...
vector<vector<RuleExecution::Node*> > putCombinations(
	...
	for (unsigned i = 0; i < combinations.size(); i++) {
		for (unsigned j = 0; j < nestedcombinations.size(); j++) {
			vector<RuleExecution::Node*> newcombination = vector<
					RuleExecution::Node*>(combinations[i]);
...
void RuleExecution::getOuts(vector<string>* finalOuts,
		vector<vector<Node*> >* finalCombNodes,
...
vector<vector<float> > RuleExecution::normaliseWeights(
...
void RuleExecution::normaliseWeights(vector<float>* weights) {
	// get sum of weights of an ambigInfo
	float sum = 0;
	for (unsigned k = 0; k < weights->size(); k++) {
		sum += (*weights)[k];
...
void RuleExecution::normaliseWeights(vector<vector<float> >* vweights,
...
//      vector<float> weights = vweights[i];
		...
		for (unsigned j = 0; j < (*vambigInfo)[i].size(); j++) {
			// get sum of weights of an ambigInfo
			float sum = 0;
			for (unsigned k = 0; k < (*vambigInfo)[i][j]->combinations.size();
					k++) {
...
unsigned RuleExecution::getAmbigCount(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules,
		map<unsigned, vector<RuleExecution::Node*> > nodesPool) {
...
void RuleExecution::getAmbigInfo(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules,
		map<unsigned, vector<RuleExecution::Node*> > nodesPool,
...
map<unsigned, vector<RuleExecution::Node*> > RuleExecution::getNodesPool(
		map<unsigned, vector<pair<unsigned, unsigned> > > tokenRules) {
...
string RuleExecution::noRuleOut(vector<string> analysis) {
	vector<string> out;
	// unknown word
	if (analysis[0][0] == '*')
...
vector<string> vectorToString(vector<vector<string> > vOutputs) {
...
vector<vector<unsigned> > ruleVectorToIds(
		vector<vector<vector<xml_node> > > vOutRules) {
...
vector<vector<vector<unsigned> > > putRules(
	...
	for (unsigned i = 0; i < outsRules.size(); i++) {
		for (unsigned j = 0; j < nestedOutsRules.size(); j++) {
			vector<vector<unsigned> > newRule = vector<vector<unsigned> >(
					outsRules[i]);
...
vector<vector<pair<unsigned, unsigned> > > putNestedRules(
		vector<vector<pair<unsigned, unsigned> > > outsRules,
		vector<vector<pair<unsigned, unsigned> > > nestedOutsRules) {
...
//  cout << "nestedoutsrule size = " << nestedOutsRules.size () << endl;
	...
	for (unsigned i = 0; i < outsRules.size(); i++) {
		for (unsigned j = 0; j < nestedOutsRules.size(); j++) {
			vector<pair<unsigned, unsigned> > newRule = vector<
					pair<unsigned, unsigned> >(outsRules[i]);
...
bool RuleExecution::outputs(vector<string>* outs,
...
//	{
		...
		(*outs) = putOuts((*outs),
				nestedOutputs/*, i + maxPatterns - 1, spaces*/);
...
void RuleExecution::weightIndices(vector<vector<unsigned> >* weigInds,
		vector<
				pair<pair<unsigned, unsigned>,
						pair<unsigned, vector<vector<unsigned> > > > > ambigInfo,
		vector<vector<vector<unsigned> > > outsRules) {
	for (unsigned i = 0; i < ambigInfo.size(); i++) {
		pair<pair<unsigned, unsigned>,
				pair<unsigned, vector<vector<unsigned> > > > p1 = ambigInfo[i];
...
void printNodeAttrs(xml_node node) {
//  cout << node.name () << endl;
...
void RuleExecution::ruleOuts(map<unsigned, map<unsigned, string> >* ruleOuts,
		map<unsigned, vector<pair<unsigned, unsigned> > >* tokenRules,
...
//		  << tlTokens[tokInd] << "   " << noRuleOut (tlAnalysisToken) << endl;
			...
			if (rule.attribute(ID).as_uint() == 0)
				...
				output = RuleExecution::ruleExe(rule, &slAnalysisTokens,
						&tlAnalysisTokens, attrs, lists, vars, spaces,
						firstMatTok, localeId); // first pattern index
...
vector<string> RuleExecution::ruleExe(xml_node rule,
...
vector<string> RuleExecution::out(xml_node out,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
	...
	for (xml_node child = out.first_child(); child; child =
			child.next_sibling()) {
		vector<string> result;
...
vector<string> RuleExecution::chunk(xml_node chunkNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
	...
	for (xml_node child = tags.next_sibling(); child; child =
			child.next_sibling()) {
//      printNodeAttrs (child);
		...
		if (childName == LU) {
			...
			for (xml_node lu = child.first_child(); lu; lu =
					lu.next_sibling()) {
...
vector<string> RuleExecution::callMacro(xml_node callMacroNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
...
//      << endl;
	...
	unsigned i = 1;
	for (xml_node with_param = callMacroNode.child(WITH_PARAM); with_param;
...
//  cout << callMacroNode.root ().name () << endl;
	...
	for (xml_node child = macro.first_child(); child; child =
			child.next_sibling()) {
		vector<string> result;
...
vector<string> RuleExecution::choose(xml_node chooseNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
	...
	for (xml_node child = chooseNode.first_child(); child;
			child = child.next_sibling()) {
...
//      cout << "condition=" << condition << endl;
		if (condition) {
			for (xml_node inst = child.first_child(); inst;
...
bool RuleExecution::test(xml_node test,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
...
bool RuleExecution::And(xml_node andNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
	...
	for (xml_node child = andNode.first_child(); child;
			child = child.next_sibling()) {
		string childName = child.name();
		if (childName == EQUAL) {
			condition = equal(child, slAnalysisTokens, tlAnalysisTokens, attrs,
...
bool RuleExecution::Or(xml_node orNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
	...
	for (xml_node child = orNode.first_child(); child;
			child = child.next_sibling()) {
		string childName = child.name();
		if (childName == EQUAL) {
			condition = equal(child, slAnalysisTokens, tlAnalysisTokens, attrs,
...
bool RuleExecution::in(xml_node inNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
...
bool RuleExecution::Not(xml_node NotNode,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs,
		map<string, vector<string> > lists, map<string, string>* vars,
...
vector<string> RuleExecution::litTag(xml_node litTag) {
...
string RuleExecution::lit(xml_node lit) {
...
string RuleExecution::var(xml_node var, map<string, string>* vars) {
...
void RuleExecution::let(xml_node let, vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
		map<unsigned, unsigned> paramToPattern) {
...
vector<string> RuleExecution::clip(xml_node clip,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
...
void RuleExecution::append(xml_node append,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
	...
	for (xml_node child = append.first_child(); child;
			child = child.next_sibling()) {
		string childName = child.name();
		if (childName == CLIP) {
...
string RuleExecution::b(xml_node b, vector<string> spaces, unsigned firPat,
...
string RuleExecution::caseOf(xml_node caseOf,
		vector<vector<string> >* slAnalysisTokens,
...
string RuleExecution::getCaseFrom(xml_node getCaseFrom,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
...
void RuleExecution::modifyCase(xml_node modifyCase,
		vector<vector<string> >* slAnalysisTokens,
		vector<vector<string> >* tlAnalysisTokens,
		map<string, vector<vector<string> > > attrs, map<string, string>* vars,
		vector<string> spaces, unsigned firPat, string localeId,
